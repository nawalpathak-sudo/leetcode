...
export async function generateUniqueUsername(name) {
  const base = (name || '').toLowerCase().trim().replace(/[^a-z0-9]+/g, '').slice(0, 20)
  if (!base) return null
  // Check if base is available
  const { data } = await supabase
    .from('students')
    .select('student_username')
    .ilike('student_username', `${base}%`)
  const taken = new Set((data || []).map(s => s.student_username))
...
export async function updateStudentPhone(leadId, phone) {
  // Also auto-generate username if missing
  const { data: student } = await supabase
    ...
    .single()
  const updates = { phone }
  if (student && !student.student_username && student.student_name) {
    const username = await generateUniqueUsername(student.student_name)
    if (username) updates.student_username = username
  }
  const { error } = await supabase
    .from('students')
    .update(updates)
