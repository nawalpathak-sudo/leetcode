...
import { Plus, Trash2, Edit3, Upload, Loader2, X, Check, AlertCircle, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function ProblemsClient({ initialProblems, totalCount }: ProblemsClientProps) {
  ...
  const fetchPage = useCallback(async (pageNum: number, topicFilter?: string, difficultyFilter?: string) => {
    setLoading(true)
    ...
    const activeTopic = topicFilter !== undefined ? topicFilter : filterTopic
    const activeDiff = difficultyFilter !== undefined ? difficultyFilter : filterDifficulty

    if (activeTopic) query = query.eq('topic', activeTopic)
    if (activeDiff) query = query.eq('difficulty', activeDiff)
  ...
  const applyFilter = (newTopic: string, newDifficulty: string) => {
    setFilterTopic(newTopic)
    setFilterDifficulty(newDifficulty)
    setPage(0)
    fetchPage(0, newTopic, newDifficulty)
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold" style={{ color: 'var(--color-primary)' }}>LeetCode Problems</h2>
        <p className="mt-1" style={{ color: 'var(--color-text-secondary)' }}>
          Manage practice problems -- paste LeetCode links and details are fetched automatically.
        </p>
      </div>
      ...
      {tab === 'add' && (
        <div className="space-y-6">
          ...
          <div className="flex gap-3 items-center flex-wrap">
            <select value={filterTopic} onChange={e => applyFilter(e.target.value, filterDifficulty)}
              className="border rounded-lg px-3 py-2 text-sm focus:outline-none"
              style={{ borderColor: 'var(--color-border)', color: 'var(--color-primary)' }}>
              <option value="">All Topics</option>
              {TOPICS.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
            <select value={filterDifficulty} onChange={e => applyFilter(filterTopic, e.target.value)}
              className="border rounded-lg px-3 py-2 text-sm focus:outline-none"
              style={{ borderColor: 'var(--color-border)', color: 'var(--color-primary)' }}>
            ...
            {(filterTopic || filterDifficulty) && (
              <button onClick={() => applyFilter('', '')}
                ...
                style={{ color: 'var(--color-text-secondary)', background: 'rgba(13,30,86,0.03)' }}>
                    ...
                    {items.map((p: any) => editId === p.id ? (
                      <div key={p.id} className="px-5 py-3 space-y-3 bg-amber-50/30">
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                          ...
                          {editForm.topic === '__custom__' && (
                            <input value={editForm.customTopic} onChange={e => setEditForm((f: any) => ({ ...f, customTopic: e.target.value }))}
                              className="border rounded px-2 py-1.5 text-xs" style={{ borderColor: 'var(--color-border)' }} placeholder="Custom topic" />
                          )}
                      ...
                      <div key={p.id} className="px-5 py-3 flex items-center gap-3 hover:bg-[rgba(13,30,86,0.01)] transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {p.notes && <span className="text-xs ml-2" style={{ color: 'var(--color-text-secondary)' }}> -- {p.notes}</span>}
                        </div>
